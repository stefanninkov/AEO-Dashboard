...
import {
  ...
  Share2, Copy, Loader2, Link2, X, Mail, Send, Plug, RefreshCw, Unplug, Zap, Plus
} from 'lucide-react'
...
import { WEBHOOK_EVENT_GROUPS, testWebhook as testWebhookUrl } from '../utils/webhookDispatcher'
          ...
          <div className="card" style={{ marginBottom: '1rem' }}>
            ...
            <div style={{ padding: '0.875rem 1.25rem', borderBottom: '1px solid var(--border-subtle)' }}>
              ...
              <div style={{
                ...
                background: 'rgba(14, 165, 233, 0.06)', borderRadius: '0.5rem',
                border: '1px solid rgba(14, 165, 233, 0.12)', fontSize: '0.6875rem',
                ...
                <div style={{ display: 'flex', gap: '0.625rem', flexWrap: 'wrap' }}>
                  <div style={{ flex: 1, minWidth: '10rem' }}>
                    ...
                    <input
                      ...
                      placeholder="e.g. Slack — Phase Alerts"
                ...
                <div>
                  ...
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.375rem' }}>
                    <label style={{
                      ...
                      background: webhookEvents.includes('*') ? 'rgba(255, 107, 53, 0.1)' : 'var(--hover-bg)',
                      border: `1px solid ${webhookEvents.includes('*') ? 'var(--color-phase-1)' : 'var(--border-subtle)'}`,
                    }}>
                      <input
                        type="checkbox"
                        checked={webhookEvents.includes('*')}
                        onChange={(e) => setWebhookEvents(e.target.checked ? ['*'] : [])}
                        style={{ accentColor: 'var(--color-phase-1)' }}
                      />
                      All Events
                    </label>
                    {!webhookEvents.includes('*') && Object.entries(WEBHOOK_EVENT_GROUPS).map(([key, group]) => (
                      <label key={key} style={{
                        ...
                        fontSize: '0.75rem', color: 'var(--text-secondary)', cursor: 'pointer',
                        padding: '0.25rem 0.5rem', borderRadius: '0.375rem',
                        background: webhookEvents.includes(key) ? 'rgba(14, 165, 233, 0.08)' : 'var(--hover-bg)',
                        border: `1px solid ${webhookEvents.includes(key) ? 'var(--color-phase-3)' : 'var(--border-subtle)'}`,
                      }}>
                        <input
                          type="checkbox"
                          ...
                          onChange={(e) => {
                            if (e.target.checked) {
                              ...
                              setWebhookEvents(webhookEvents.filter(ev => ev !== key))
                            }
                          }}
                          style={{ accentColor: 'var(--color-phase-3)' }}
                        />
                        {group.label}
                      </label>
                    ))}
            ...
            {webhooks.map((webhook, idx) => (
              <div key={webhook.id} style={{
                ...
                borderBottom: idx < webhooks.length - 1 ? '1px solid var(--border-subtle)' : 'none',
              }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.625rem', flexWrap: 'wrap' }}>
                  {/* Status dot */}
                  <span style={{
                    width: 8, height: 8, borderRadius: '50%', flexShrink: 0,
                    background: !webhook.enabled ? 'var(--text-disabled)'
                      : webhook.lastStatus === 'success' ? 'var(--color-success)'
                      : webhook.lastStatus === 'error' ? 'var(--color-error)'
                      : 'var(--color-phase-5)',
                  }} />

                  {/* Name + URL */}
                  <div style={{ flex: 1, minWidth: '8rem' }}>
                    <span style={{ fontSize: '0.8125rem', fontWeight: 600, color: webhook.enabled ? 'var(--text-primary)' : 'var(--text-tertiary)' }}>
                      {webhook.name || 'Untitled'}
                    </span>
                    <div style={{ fontSize: '0.625rem', color: 'var(--text-disabled)', fontFamily: 'var(--font-heading)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: '18rem' }}>
                      {webhook.url}
                    </div>
                  </div>
                  ...
                  <span style={{
                    ...
                    background: webhook.format === 'slack' ? 'rgba(74, 21, 75, 0.1)' : webhook.format === 'discord' ? 'rgba(88, 101, 242, 0.1)' : 'var(--hover-bg)',
                    color: webhook.format === 'slack' ? '#4A154B' : webhook.format === 'discord' ? '#5865F2' : 'var(--text-tertiary)',
                  }}>
                    {webhook.format}
                  </span>
                  ...
                  {/* Event count badge */}
                  <span style={{
                    fontSize: '0.625rem', fontWeight: 600, fontFamily: 'var(--font-heading)',
                    padding: '0.125rem 0.375rem', borderRadius: '0.625rem',
                    background: 'rgba(14, 165, 233, 0.08)', color: 'var(--color-phase-3)',
                  }}>
                    {webhook.events.includes('*') ? 'All events' : `${webhook.events.length} event${webhook.events.length !== 1 ? 's' : ''}`}
                  </span>

                  {/* Toggle */}
                  <ToggleSwitch checked={webhook.enabled} onChange={() => toggleWebhook(webhook.id)} />
                  ...
                  >
                    {webhookTesting === webhook.id ? <Loader2 size={11} className="spin" /> : <Send size={11} />}
                    Test
                  </button>
                  ...
                  <button
                    ...
                    onClick={() => {
                      ...
                      setWebhookFormat(webhook.format || 'json')
                  ...
                  <button
                    style={{
                      ...
                      color: deleteWebhookConfirm === webhook.id ? 'var(--color-error)' : 'var(--text-tertiary)',
                      transition: 'color 100ms',
                    }}
                    onClick={() => {
                      if (deleteWebhookConfirm === webhook.id) {
                        removeWebhook(webhook.id)
                        setDeleteWebhookConfirm(null)
                        addToast('success', 'Webhook removed')
                      } else {
                        setDeleteWebhookConfirm(webhook.id)
                        setTimeout(() => setDeleteWebhookConfirm(null), 3000)
                      }
                    }}
