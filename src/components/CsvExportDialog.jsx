...
const { addToast } = useToast()
  ...
  const SECTIONS = useMemo(() => [
    { key: 'checklist', label: 'Checklist Data', desc: 'Task status, phases, notes, and assignments', default: true },
    { key: 'metrics', label: 'Metrics History', desc: 'Score and performance data over time', default: true, dataKey: 'metricsHistory' },
    { key: 'activity', label: 'Activity Log', desc: 'Recent activity and team actions', default: false, dataKey: 'activityLog' },
    { key: 'competitors', label: 'Competitors', desc: 'Competitor URLs, scores, and analysis', default: false, dataKey: 'competitors' },
  ], [])
  ...
  const handleGenerate = () => {
    ...
    try {
      ...
      if (exported.length > 0) {
        addToast('success', `${exported.length} CSV file(s) downloaded!`)
    ...
    <div className="email-modal-backdrop" onClick={onClose}>
      ...
      >
        ...
        <div className="email-modal-actions">
          ...
          >
            <Download size={14} />
            {generating ? 'Exporting...' : (selectedCount !== 1 ? `Download ${selectedCount} CSVs` : `Download ${selectedCount} CSV`)}
          </button>
